...
class BungMaking implements Runnable {
    ...
    public void run() {
        ...
        while (BungCount > 0) {     // 남은 붕어빵 틀이 0이 될 때까지 반복
            ...
            try {
                Thread.sleep(Math.round(1000 * Math.random()));     // 스레드마다 다음 붕어빵을 만들기까지 0초에서 1초까지 랜덤으로 시간 걸리도록 함(붕어빵 마다 만드는 시간 상이
